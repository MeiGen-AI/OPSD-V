...
class CausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            use_relative_sink = bool(kv_cache.get("use_relative_sink", False)) if kv_cache is not None else False
            history_only_after_first_gt = bool(kv_cache.get("history_only_after_first_gt", False)) if kv_cache is not None else False
            use_history_only_attention = history_only_after_first_gt and current_start > 0
            persistent_sink_raw_k = kv_cache.get("persistent_sink_raw_k") if kv_cache is not None else None
            persistent_sink_v = kv_cache.get("persistent_sink_v") if kv_cache is not None else None
            persistent_sink_num_frames = int(kv_cache.get("persistent_sink_num_frames", 0) or 0) if kv_cache is not None else 0
            persistent_sink_tokens = (
                persistent_sink_v.shape[1]
                if (persistent_sink_v is not None and persistent_sink_v.ndim >= 2)
                else sink_tokens
            )
            use_persistent_sink = (
                use_relative_sink
                and sink_tokens > 0
                and persistent_sink_raw_k is not None
                and persistent_sink_v is not None
                and persistent_sink_num_frames > 0
                and current_start_frame >= persistent_sink_num_frames
            )

            captured_sink_num_frames_buf = (
                kv_cache.get("captured_sink_num_frames_buf")
                if kv_cache is not None
                else None
            )
            sink_already_captured = (
                kv_cache is not None
                and (
                    kv_cache.get("captured_sink_raw_k") is not None
                    or (
                        captured_sink_num_frames_buf is not None
                        and int(captured_sink_num_frames_buf.item()) > 0
                    )
                )
            )
            if (
                kv_cache is not None
                and kv_cache.get("capture_sink_raw", False)
                and (current_start == 0 or kv_cache.get("capture_sink_raw_any_start", False))
                and sink_tokens > 0
                and k.shape[1] >= sink_tokens
                and not sink_already_captured
            ):
                k_buf = kv_cache.get("captured_sink_raw_k_buf", None)
                v_buf = kv_cache.get("captured_sink_v_buf", None)
                num_frames_buf = kv_cache.get("captured_sink_num_frames_buf", None)
                if (
                    k_buf is not None
                    and v_buf is not None
                    and num_frames_buf is not None
                    and k_buf.shape == k[:, :sink_tokens].shape
                    and v_buf.shape == v[:, :sink_tokens].shape
                ):
                    k_buf.copy_(k[:, :sink_tokens].detach())
                    v_buf.copy_(v[:, :sink_tokens].detach())
                    num_frames_buf.fill_(int(self.sink_size))
                else:
                    kv_cache["captured_sink_raw_k"] = k[:, :sink_tokens].detach().clone()
                    kv_cache["captured_sink_v"] = v[:, :sink_tokens].detach().clone()
                    kv_cache["captured_sink_num_frames"] = int(self.sink_size)
            if (
                kv_cache is not None
                and kv_cache.get("capture_context_raw", False)
                and kv_cache.get("captured_context_raw_k") is None
            ):
                kv_cache["captured_context_raw_k"] = k.detach().clone()
                kv_cache["captured_context_v"] = v.detach().clone()
                kv_cache["captured_context_num_frames"] = int(grid_sizes[0, 0].item())
            ...
            attn_local_end_index = local_start_index if use_history_only_attention else local_end_index
            future_context_raw_k = kv_cache.get("future_context_raw_k") if kv_cache is not None else None
            future_context_v = kv_cache.get("future_context_v") if kv_cache is not None else None
            future_context_num_frames = int(kv_cache.get("future_context_num_frames", 0) or 0) if kv_cache is not None else 0
            future_context_start_frame = int(kv_cache.get("future_context_start_frame", 0) or 0) if kv_cache is not None else 0
            use_future_context = (
                future_context_raw_k is not None
                and future_context_v is not None
                and future_context_num_frames > 0
            )
            if use_future_context:
                future_grid_sizes = grid_sizes.clone()
                future_grid_sizes[:, 0] = future_context_num_frames
                future_context_k = causal_rope_apply(
                    future_context_raw_k,
                    future_grid_sizes,
                    freqs,
                    start_frame=future_context_start_frame,
                ).type_as(v)
                future_context_v_cat = future_context_v.type_as(v)
            if sink_tokens > 0:
                # Concatenate sink tokens and local window tokens, keeping total length strictly below max_attention_size
                sink_token_count = persistent_sink_tokens if use_persistent_sink else sink_tokens
                local_budget = self.max_attention_size - sink_token_count
                k_sink = temp_k[:, :sink_tokens]
                v_sink = temp_v[:, :sink_tokens]
                # if (not dist.is_initialized() or dist.get_rank() == 0) and DEBUG:
                #     print(f"local_budget: {local_budget}")
                if local_budget > 0:
                    local_start_for_window = max(sink_tokens, attn_local_end_index - local_budget)
                    k_local = temp_k[:, local_start_for_window:attn_local_end_index]
                    v_local = temp_v[:, local_start_for_window:attn_local_end_index]
                else:
                    local_start_for_window = attn_local_end_index
                    k_local = temp_k[:, local_start_for_window:attn_local_end_index]
                    v_local = temp_v[:, local_start_for_window:attn_local_end_index]

                if use_persistent_sink:
                    blend_alpha = float(kv_cache.get("dynamic_sink_alpha", 0.0) or 0.0)
                    blend_alpha = max(0.0, min(1.0, blend_alpha))
                    dynamic_sink_raw_k = kv_cache.get("dynamic_sink_raw_k")
                    dynamic_sink_v = kv_cache.get("dynamic_sink_v")
                    sink_raw_for_rope = persistent_sink_raw_k
                    sink_v_for_cat = persistent_sink_v.type_as(v)
                    if (
                        blend_alpha > 0.0
                        and dynamic_sink_raw_k is not None
                        and dynamic_sink_v is not None
                        and dynamic_sink_raw_k.shape == persistent_sink_raw_k.shape
                        and dynamic_sink_v.shape == persistent_sink_v.shape
                    ):
                        sink_raw_for_rope = (
                            (1.0 - blend_alpha) * persistent_sink_raw_k
                            + blend_alpha * dynamic_sink_raw_k
                        )
                        sink_v_for_cat = (
                            (1.0 - blend_alpha) * persistent_sink_v.type_as(v)
                            + blend_alpha * dynamic_sink_v.type_as(v)
                        )

                    history_token_count = max(0, local_start_index - local_start_for_window)
                    working_cache_frame_length = history_token_count // frame_seqlen
                    sink_start_frame = max(
                        0,
                        current_start_frame - working_cache_frame_length - persistent_sink_num_frames,
                    )
                    sink_grid_sizes = grid_sizes.clone()
                    sink_grid_sizes[:, 0] = persistent_sink_num_frames
                    dynamic_sink_k = causal_rope_apply(
                        sink_raw_for_rope,
                        sink_grid_sizes,
                        freqs,
                        start_frame=sink_start_frame,
                    ).type_as(v)
                    k_sink = dynamic_sink_k
                    v_sink = sink_v_for_cat

                if use_future_context:
                    k_cat = torch.cat([k_sink, future_context_k, k_local], dim=1)
                    v_cat = torch.cat([v_sink, future_context_v_cat, v_local], dim=1)
                else:
                ...
                window_start = max(0, attn_local_end_index - self.max_attention_size)
                if use_future_context:
                    k_cat = torch.cat([future_context_k, temp_k[:, window_start:attn_local_end_index]], dim=1)
                    v_cat = torch.cat([future_context_v_cat, temp_v[:, window_start:attn_local_end_index]], dim=1)
                    x = attention(
                        roped_query,
                        k_cat,
                        v_cat
                    ...
                    x = attention(
                        roped_query,
                        temp_k[:, window_start:attn_local_end_index],
                        temp_v[:, window_start:attn_local_end_index]
                    )
