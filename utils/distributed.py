...
class EMA_FSDP:
    ...
    def _init_shadow(self, fsdp_module):
        ...
        with FSDP.summon_full_params(fsdp_module, writeback=False, rank0_only=False):
            for n, p in fsdp_module.named_parameters():
                self.shadow[self._clean_param_name(n)] = p.detach().clone().float().cpu()
        if not self.shadow:
            rank = dist.get_rank() if dist.is_initialized() else 0
            print(f"[EMA_FSDP DEBUG] rank={rank} initialized empty EMA shadow")
    ...
    def load_state_dict(self, sd):
        self.shadow = {self._clean_param_name(k): v.clone() for k, v in sd.items()}

    def copy_to(self, fsdp_module):
        # load EMA weights into an (unwrapped) copy of the generator
        from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
        with FSDP.summon_full_params(fsdp_module, writeback=True, rank0_only=False):
            for n, p in fsdp_module.named_parameters():
                n = self._clean_param_name(n)
                if n in self.shadow:
